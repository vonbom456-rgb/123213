...
Four ArkApi 3.56 plugins, one repo, one build:
...
- **CryoRecovery/** — configurable cryopod healing with a slightly slower RAID/PvP rate
...
репозиторий; скрипт загрузит все четыре проекта и откроет страницу Actions.
...
four `.sln` in one job, and uploads a single artifact **AllPlugins-Release-x64**
containing:

```
TurretControl/TurretControl.dll + config.json + PluginInfo.json
PvPCooldowns/PvPCooldowns.dll   + config.json + PluginInfo.json
DamageNumbers/DamageNumbers.dll + config.json + PluginInfo.json
CryoRecovery/CryoRecovery.dll   + config.json + PluginInfo.json
```
