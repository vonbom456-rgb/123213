...
void Scan(){
 ...
 TArray<AActor*> drops;UGameplayStatics::GetAllActorsOfClass(world,TSubclassOf<AActor>(ADroppedItem::GetPrivateStaticClass()),&drops);
 for(AActor* actor:drops){if(removed>=g_config.max_per_scan)break;if(!actor||!actor->IsA(ADroppedItem::GetPrivateStaticClass()))continue;seen.insert(actor);auto* drop=static_cast<ADroppedItem*>(actor);UPrimalItem* item=drop->MyItemField();if(item&&item->GetItemQuantity()>0){g_first_empty.erase(actor);continue;}if(Ready(actor,g_config.dropped_grace,now)&&actor->Destroy(true,true)){g_first_empty.erase(actor);++removed;}}
